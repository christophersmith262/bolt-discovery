...
class Pager {
  ...
  constructor(total, req, query) {
    ...
    this.remaining = this.total - this.offset;
  }
  ...
  first() {
    return this.baseUrl + this.path.replace(/[?&]page\[offset\]=[0-9]+/, '');
  }
  ...
  page(offset) {
    ...
    if (!offset) {
      ...
      let path = this.path.replace(/([?&])page\[offset\]=[0-9]+/, '$1page[offset]=' + offset);
